...
We want to prove that the C code implementation of the respects rules extracted from RFC 793. To achieve this, symbolic execution is used to verify that all the possible paths when executing the \spark{tcpProcessSegment} function will result in a state that respects the RFC 793 rules. This process requires three steps, that are show in \Cref{code:kleedriver}. A symbolic incoming segment is created in the first lines to allow us to account for all possible incoming segments. Then at \cref{code:kleedriver:tcpProcessSegment} the function \texttt{tcpProcessSegment} is executed symbolically with KLEE. Finally, it is followed at \cref{code:kleedriver:assert} by a call to \texttt{klee\_assert} that checks if the code leads to a valid state identified in the protocol’s specification.

Once the \texttt{klee\_assert} is validated by KLEE, which means that the code respects the RFC 793 specification, it can be copied out as a valid SPARK contract for the function \spark{TCP_Process_One_Segment}. With this step done, the concurrency challenge has been solved and the function \spark{TCP_Wait_For_Events} has a correct and an accurate contract, which makes possible the verification of the functional correctness of the TCP user
