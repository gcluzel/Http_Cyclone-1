...
The CycloneTCP implementation adopts the three-task-based model and the socket data structure. Thus, one task is dedicated to the user's code, one to the timers, and one to the processing of the incoming segments. The user can manipulate a socket through the TCP's library Application Programming Interface (API) to perform various TCP operations, such as data transmission.
