...
In the CycloneTCP library, pointers are used both as the type of variables, to pass objects when calling other functions, and the type of structure fields, for instance in
the socket structure to store other complex data-structures and recursive pointer-based data-structures. SPARK use a pointer ownership model to prevent bugs
involving pointers such as memory leak or double free, which imposes some restrictions on how pointers are manipulated.
When a memory area is allocated, a pointer points to this memory area and thus it is owned.
...
The socket structure is shared between different functions, and contains lots
of fields. Most of the fields are used in other parts of the library and are not
relevant for our verification. Thus we use \emph{ghost code} in SPARK
to focus the verification effort on the relevant subset of the fields,
which also makes the specifications easier to write and to understand.
This is a solution to the classical problem of specifying the \emph{frame condition} for a function, that is, the set of possibly modified objects.
Ghost code is used by GNATprove for analysis but not compiled in the final executable.

More specifically, we define a \emph{model} of the actual socket as a ghost function for use in contracts: \spark{Model(Sock)}
extracts the fields of interest from socket \spark{Sock}.
A frequent usage in postconditions is
...
not changed by the function call, except for the field \spark{State} that is equal
...
is mandatory for provide functional contracts to the user functions. That can
be achieved by reading the text of RFC 793. However, the text is underspecified in many
cases and only one possible implementation is described. We made the following choices in our formalization of the TCP specification:
\begin{itemize}
...
these two states.
\item RFC 793 describes conditions on the state of the socket for calling user functions,
which need to hold when the function is called~\cite[p. 52]{rfc793}. In particular it is specified in what state the
function can be called without returning an error. These became preconditions in our formalization.
\end{itemize}

\subsection{Rewriting the TCP user functions}

The user functions defining the API of the TCP protocol were fully rewritten in SPARK.
SPARK bindings were also provided to the majority of the rest of the library’s C functions to allow their invocation from the SPARK code,
taking into account their effect on the TCP state machine in their contracts.
GNATprove was used to prove that the implementation of user functions is free of run-time errors and
respects the functional specifications of the TCP protocol that we formalized in contracts.
...
To model transitions of the TCP state machine inside the code, we defined the helper
function \spark{TCP_Change_State}, whose contract describes
all the transitions allowed.
As calls to user functions are protected with a mutex to avoid interferences
from other threads, the only changes to the TCP state machine inside user functions are
related to their actions. We model these changes by calling \spark{TCP_Change_State}
at places that correspond to a transition of the TCP machine.
...
The function \spark{TCP_Wait_For_Events} requires special handling because it may
release the mutex. This function takes in argument an event on which it should wait,
and returns once the event is triggered by the reception of a segment.
While waiting, it releases the mutex to allow the reception task to process incoming segments.
For example, this function is called from \spark{open} after the connection
has been initialized, to wait until the connection has been established or not, in order to
correctly set the error that indicates to the user the state of the connection.

% Yannick: we talk about error above, but there is no error if the connection is established???
% This needs rephrasing

A correct but imprecise contract for \spark{TCP_Wait_For_Events} would allow any change in the TCP
state machine while waiting. But it would make it impossible to prove that its callers
satisfy their functional specification. We need a more precise contract for this function.
...
received or until the timeout is reached. Each time a new segment is received, it
...
Rewriting the part that processes the incoming segments was out of the scope of
...
the authors explain that symbolic execution can be good at inferring contracts.
...
builtin that checks if an assert is valid for all the symbolic paths
explored before.
...
% above: need time to prove automatically with GNATprove with the spec of your laptop (num of cores, amount of memory)
