...
The pointer ownership policy of SPARK, detailed in \Cref{sec:pointers}, enabled the detection of a memory leak when a socket closes. This was present due to partially freeing the memory allocated for the buffer that stores the incoming segments. This bug was due to using the wrong library function call to free the memory. SPARK ownership policy detected this because, after the call to this function, the pointer to the buffer was declared not own anymore, but the memory pointed by that pointer was partially deallocated.

\subsubsection{Violation of the TCP protocol}

A subtle bug that violates the protocol's extracted specifications captured by SPARK is reproduced below:
...
These transitions are not allowed by RFC 793, and thus, not allowed by the precondition of \spark{TCP_Change_State}, which models all the allowed transitions by the protocol's specification. The problem was both corrected in the C original implementation and the SPARK translated code.
